...
def test_copy_page_sink_tail():
    ...
    # (dst groups 2,3 <- src groups 0,1). Blocks are slot-windowed:
    # resolve through the rev maps (raw indices are slots now).
    assert torch.equal(dst.exact_k[int(dst._exact_rev[2])],
                       layer.exact_k[int(layer._exact_rev[0])])
    assert torch.equal(dst.exact_v[int(dst._exact_rev[3])],
                       layer.exact_v[int(layer._exact_rev[1])])
    ...
    # resolved through the rev maps).
    dst2 = _layer(2, 128, 512)
    dst2.copy_page(layer, 1, 0, 44)
    assert not bool(dst2.sealed[0])
    assert torch.equal(dst2.exact_k[int(dst2._exact_rev[0])][:44],
                       layer.exact_k[int(layer._exact_rev[2])][:44])
    assert torch.equal(dst2.stage_v[int(dst2._stage_rev[0])][:44],
                       layer.stage_v[int(layer._stage_rev[2])][:44])
