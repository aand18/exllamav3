...
| --- | --- | --- | --- |
| `wip/kvarn-cache` | — (feature port, no upstream issue) | WIP: 4090-validated, 35.2 tok/s decode @8192 (27B dense 1.4bpw, fp16 85–86 same runs); micro-KLD identical same-top 100%, PARITY=1 clean, CPU 73 + triton twins green; optimizing the generation wall | [#2 (draft)](https://github.com/aand18/exllamav3/pull/2) |
...
on the branch: prefill 134.6s → ~5.0s, decode 8.9 → 35.2 tok/s @8192
(27B dense 1.4bpw, fp16 85–86 same runs) via fused Triton serve/store
kernels, sync-free evict scan, global dirty sweep + in-place WHT, and
steady-path call trimming. Micro-KLD vs fp16 identical (same-top
100%); `EXL3_KVARN_TRITON_PARITY=1` clean at 8k; CPU suite 73 passed;
triton twins 9 passed 1 skipped. Current wall is per-step launch count
(Kineto: host-starved, Self CPU ≫ Self CUDA), not syncs.

**Test plan:**
- CPU unit/parity/accounting suite: `tests/test_kvarn_*.py` — 73 green
  (TRITON unset; triton tests CUDA-gated).
...
(decode 35.2 tok/s @8192, next target: per-step launch count).
