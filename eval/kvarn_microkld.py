...
fp16-cache forward of the same prompt. Prefill (not decode) keeps this
tractable: one forward pass per cache type. NTOK should exceed 128 so KVarN
actually seals a group; the tail floor keeps the suffix exact on both sides,
so this measures body-format difference, not suffix policy.
...
def run(model, cache, ids):
    logits = model.forward(ids, {"cache": cache, "attn_mode": "flash_attn",
...
def main():
    ...
    ids = tokenizer.encode(SAMPLER_TEXT * 12)[:, :args.ntok]
    ...
    print(f"KLD kvarn{k_bits}/kvarn{v_bits} vs fp16-cache "
          f"over {args.ntok} prefill positions:", flush=True)
